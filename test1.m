...
imd_mask = (imd == 0);
subplot(2,2,1);imagesc(imd_mask); axis equal; axis off;

% Fill in missing values
imd_fill = double(imd);
imd_fill = medianfillmissing(imd_fill);
subplot(2,2,2);imagesc(imd_fill); axis equal; axis off;

% Run Canny edge detector
im1 = im ./ 255;
pb = pbCanny(im1);
%pb = pb .* double(imd_mask);
subplot(2,2,3);imagesc(pb); axis equal; axis off;

% Compute & Show superpixels
img_sup = computeSupix(im, pb);
img_sup_show = segImage(im, img_sup);
subplot(2,2,4);imagesc(img_sup_show); axis equal; axis off;
